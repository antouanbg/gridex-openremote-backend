...
let bootstrapClientId, bootstrapResourceId;
if (cfg.adminPassword) {
  ...
  const roles=await kreq('/clients/'+(await kreq('/clients?clientId=openremote'))[0].id+'/roles');
  ...
  const user=await kreq('/clients/'+client.id+'/service-account-user');const selected=['read:assets','write:assets'].map(n=>roles.find(r=>r.name===n));if(selected.some(x=>!x))throw Error('OpenRemote setup roles unavailable');
  await kreq('/users/'+user.id+'/role-mappings/clients/'+(await kreq('/clients?clientId=openremote'))[0].id,'POST',selected);
