...
test('global administration is granted only to configured verified subject, never by email or realm role', () => {
  ...
  const identity={subject:owner,realm:'gridex',email:'owner@example.com',emailVerified:true,roles:['admin']};
  ...
  assert.equal(withMembershipRoles({...identity,realm:'other'},[],configured).permissions.includes('platform:manage'),false);
