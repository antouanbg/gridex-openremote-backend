...
export function validateHistoryBindings(input){
 ...
 for(const b of input){
  if(!uuid.test(b.siteId)||!uuid.test(b.gatewayId)||!/^[A-Za-z0-9]{22}$/.test(b.assetId)
   ||typeof b.topic!=='string'||/[+#\s]/.test(b.topic)||!b.topic.endsWith('/health')
   ||!b.sourceGateway||b.metric!=='cpuTemperatureC'||seen.has(b.topic))throw Error('Invalid history binding');
  seen.add(b.topic);
 }
 return input;
}
